                    ...
                    if config.lr_decay:
                        ...
                        if config.lr_warmup and self.tokens < config.warmup_tokens:
                            ...
                            progress = float(self.tokens - baseline) / float(
                                max(1, config.final_tokens - baseline)
                            )
        ...
        for epoch in range(config.epochs):
            ...
            if good_model:
                if config.mode == "Pretraining":
                    torch.save(
                        self.model.state_dict(), self.config.pretrining_checkpoint_path
                    )
...
def train_GPT(
    ...
    mconf = GPTConfig(
        ...
        final_tokens=config_dict["epochs"]
    ...
    if log_wandb:
        wandb.init(
            project=config_dict["wandb_project"], name=config_dict["wandb_runname"]
        )
